...
from sympy import Eq, S, Symbol, lambdify, simplify, sympify
...
st.markdown(
    ...
    <link
      rel="stylesheet"
      href="https://fonts.googleapis.com/css2?family=Nunito:wght@600;700&family=Open+Sans:wght@400;600&family=Ubuntu:wght@500;700&display=swap"
    />
    <style>
        .main {{
...
def extract_math_expression(problem: str) -> str:
    ...
    text = re.sub(r"[^0-9x+\-*/().=<>\ssqrt]", " ", text)
    return " ".join(text.split())


def build_graph_data(problem: str) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    cleaned = clean_expression(extract_math_expression(problem))
    left, right = parse_equation(cleaned)
    if not left:
        return None, "Provide an equation like 'x^2 - 4x + 3 = 0' to plot."
    try:
        lhs = sympify(left)
        rhs = sympify(right)
        expr = lhs - rhs
        func = lambdify(x, expr, "math")
        xs = [value / 2 for value in range(-20, 21)]
        ys = []
        for value in xs:
            try:
                ys.append(func(value))
            except Exception:
                ys.append(None)
        data = pd.DataFrame({"x": xs, "y": ys})
        return data, None
    except Exception:
        return None, "Unable to plot this equation. Please check the input."
...
def solve_algebra(problem: str) -> Dict[str, List[str]]:
    ...
    if not left:
        if "surd" in problem.lower() or "sqrt" in problem.lower() or "simplify" in problem.lower():
            try:
                expression = sympify(cleaned)
                steps.append("Identify the surd expression to simplify.")
                steps.append(f"Expression: {expression}")
                simplified = simplify(expression)
                steps.append("Simplify using factorization and square-root rules.")
                return {"steps": steps, "answer": [f"Simplified: {simplified}"]}
            except Exception:
                return {
                    "error": [
                        "We could not parse the surd expression.",
                        "Try rephrasing: 'Simplify √50' or 'Simplify sqrt(18)'.",
                    ]
                }
        return {
            "error": [
...
if nav == "Home":
    ...
    st.markdown(
        ...
        <div class="row g-3 mt-2">
          <div class="col-md-4">
            <div class="card-lite">
              <h5><i class="fa-solid fa-calculator me-2"></i>Solver Walkthroughs</h5>
              <p class="mb-0">Step-by-step solutions for equations, surds, and inequalities.</p>
            </div>
          </div>
          <div class="col-md-4">
            <div class="card-lite">
              <h5><i class="bi bi-graph-up-arrow me-2"></i>Graph Insights</h5>
              <p class="mb-0">Plot equations to visualize roots and turning points.</p>
            </div>
          </div>
          <div class="col-md-4">
            <div class="card-lite">
              <h5><i class="fa-solid fa-medal me-2"></i>Topic Quizzes</h5>
              <p class="mb-0">Practice with instant feedback and track your best scores.</p>
            </div>
          </div>
        </div>
        """,
...
if nav == "Solver":
    ...
    with col1:
        ...
        show_graph = st.checkbox("Show equation graph (for equations in x)")
            ...
            if not problem_input.strip():
                ...
                if "error" in result:
                    ...
                    for answer in result["answer"]:
                        st.write(answer)
                if show_graph:
                    data, error = build_graph_data(problem_input)
                    if error:
                        st.info(error)
                    else:
                        st.markdown("#### Graph of f(x) = Left - Right")
...
if nav == "Resources":
    ...
    with st.expander("Algebra Essentials"):
        ...
        st.write("Surds: √(ab) = √a × √b, simplify by factoring perfect squares.")
        st.write("Graphing: Plot y = ax² + bx + c to see roots where y=0.")
        st.write("Practice: Solve x² + 7x + 12 = 0")
        st.write("Practice: Solve 5x - 3 = 2x + 9")
        st.write("Practice: Simplify √72")
